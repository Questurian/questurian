...
- **Questura has one runtime: the Linux laptop (`ssh linux-laptop`).** Decided 2026-08-13. Do not start, suggest, or verify against a localhost Questura stack, and do not ask which environment to use — it is always live. The Mac is an editor and git client only.
  ...
  - **Live is the only source of truth. Stripe test mode is NOT a second environment.** Never answer a question about how the site behaves by looking at test mode, and never "verify" a change there. If you want to know what live does, read live: `apps/questura/scripts/stripe-live` for Stripe, `ssh linux-laptop` for the app and DB, and the real domain in a browser. Test mode has its own products, prices, customers and payment-method availability, so a test-mode observation is evidence about test mode and nothing else.
