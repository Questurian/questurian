...
| `apps/location-manager` | **Location Manager** (nested monorepo) | Internal admin platform: enrich Location data (dining, accommodations, attractions, nightlife, key-locations), aggregate Google + TripAdvisor reviews, request alt text, push to Payload. | Public rendering (Questura owns that); long-form article body (AI Blog Writer). | [apps/location-manager/CONTEXT.md](./apps/location-manager/CONTEXT.md) |
| `apps/location-manager/packages/server` | LM Server | Canonical Location store (SQLite), review pipelines, image processing (Sharp), Payload sync state. | Alt-text inference; public rendering. | [.../server/CONTEXT.md](./apps/location-manager/packages/server/CONTEXT.md) |
...
| `apps/questura` | **Questura** (nested monorepo) | Public travel platform. Payload CMS backend + Next.js public site. Serves location guides, attractions, dining, accommodations, nightlife, tours, currencies, paid content. | Enrichment workflow (delegated to Location Manager); article body generation (delegated to AI Blog Writer). | [apps/questura/CONTEXT.md](./apps/questura/CONTEXT.md) |
| `apps/questura/apps/server` | Questura Server | Payload 3 collections, guide resolution, GraphQL + REST, auth, payments. | UI rendering. | [.../server/CONTEXT.md](./apps/questura/apps/server/CONTEXT.md) |
...
- **Producers:** Location Manager (sync via `/api/collections/*`); AI Blog Writer (location-documents pipeline, fills `aiFieldPaths`).
- **Consumers:** Questura client (SSR public guide pages).
- **Bridge:** `/location-guide-contract.json` at meta-root defines field paths, hierarchy resolution, and AI-fillable fields.
...
Meta-root file. Schema bridging Location Manager → Questura. Defines:
- geo hierarchy (country → city → neighborhood),
- `aiFieldPaths` (fields AI Blog Writer is allowed to fill),
- resolution precedence (neighborhood > city > country),
- array merge strategy (replace-on-non-empty).
...
- **Nested monorepos do not import each other's code.** All coupling is HTTP plus `/location-guide-contract.json`.
...
5. **Do not introduce a code-level dependency between nested monorepos.** If you need to share data, extend the contract (`location-guide-contract.json`) or go over HTTP.
6. **Use pnpm, not npm.** The user has confirmed this is a hard rule.

## Open Questions

- Should `location-guide-contract.json` formally belong to Location Manager's context (producer) rather than living unowned at meta-root?
