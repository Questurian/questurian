...
| `apps/ai-blog-writer/apps/frontend` | ABW Frontend | Operator UI: start runs, watch progress, edit Drafts, sync to Payload; staff management (invite writers/editors, promote writers, self-service Author profiles, admin editing of any Author profile). | Pipeline execution; rich-content conversion; CMS persistence; Staff identity records (Payload Users owns those). | [.../frontend/CONTEXT.md](./apps/ai-blog-writer/apps/frontend/CONTEXT.md) |
...
An optional reader-facing cue attached to a non-lodging stop in a Listicle Itinerary. It consists of a stable **Moment key** plus an optional **Moment label**; the operator surface calls it a **Moment badge**.

- **Owner of accepted Moment keys:** Questura Server.
- **Producer:** AI Blog Writer's itinerary builder.
- **Consumer:** Questura Client's public itinerary renderer.
- **Bridge:** Payload's Listicle Itinerary schema.
