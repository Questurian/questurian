  ...
  return (
    <CuratedSlotSwapProvider slots={slots} onReorder={onReorder}>
      <div className="hf-fa-spotlight">
              ...
              >
                <span className="hf-fa-spotlight-empty-title">
                  {invalidItem
                    ...
                    : `Slot ${safeIndex + 1} — choose article`}
                </span>
        ...
        {/* ── Slot indicator dots ── */}
        <div className="hf-carousel-dots hf-carousel-dots--slot-swap">
          {slots.map((slotItem, i) => {
            const slotInvalid = invalidItemsBySlot.has(i + 1)
            const filled = Boolean(slotItem)
            const dot = (
              <button
                type="button"
            ...
            if (!filled) {
              return <span key={i}>{dot}</span>
            }

            return (
              <CuratedSlotSwapWrap key={i} slotIndex={i}>
                {dot}
              </CuratedSlotSwapWrap>
            )
          })}
