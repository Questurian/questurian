...
type Props = {
  ...
  onPick: (candidate: HomepageLocationGridCandidate) => void
  onClose: () => void
  setSearchValue: (v: string) => void
  setCandidatePage: (v: number | ((prev: number) => number)) => void
}

export function LocationGridPickerModal({
  slotIndex,
  childLevel,
  candidatesQuery,
  searchValue,
  ...
  return (
    <div className="hf-modal-backdrop" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`Pick ${label} for Slot ${slotIndex + 1}`}
      >
        <div className="hf-modal-top">
          ...
          <div className="hf-modal-search">
            ...
            <input
              type="search"
              placeholder={`Search ${label} name or key…`}
              value={searchValue}
