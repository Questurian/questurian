...
function previewItems(block: PageBlockResponse): PreviewItem[] {
  ...
  return items.map((raw, index) => {
    ...
    const title =
      ...
      (typeof record.neighborhoodName === 'string' &&
...
function sectionText(
  ...
  key: 'sectionHeading' | 'sectionSubheading'
): string | null {
 ...
 */
export default function PublishedHomepagePreview({
  blocks
}: {
  ...
  if (!blocks || blocks.length === 0) {
    return (
      <div className="hf-state-screen">
        ...
        <p>
          This homepage has never been published. Publish the draft to make it
          live.
        </p>
      </div>
    )
  ...
  return (
    <div>
      ...
      >
        ...
        <span>
          Read-only preview of the published page. Switch to Draft to edit.
        </span>
      </div>
            ...
            {items.length > 0 ? (
              <ol style={{ margin: '0.6rem 0 0', paddingLeft: '1.2rem' }}>
                {items.map((item) => (
                  <li
                    key={item.key}
                    style={{ fontSize: '0.85rem', padding: '0.1rem 0' }}
                  >
                    {item.title}
                    {item.status && item.status !== 'published' && (
                      <span style={{ color: '#92400e', marginLeft: '0.4rem' }}>
                        ({item.status})
                      </span>
                    )}
                  </li>
