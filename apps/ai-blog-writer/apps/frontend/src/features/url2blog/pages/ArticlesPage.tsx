...
const STAGED_ARTICLES_KEY = 'url2blog_staged_articles_v2'
...
export default function ArticlesPage() {
  ...
  const [generatedDeleteTarget, setGeneratedDeleteTarget] = useState<Url2BlogSavedArticle | null>(null)
  ...
  const payloadStatusesQuery = useQuery({
    queryKey: ['url2blog', 'payload-statuses', payloadArticleIds.join(','), token || 'no-token'],
  ...
  const deleteGeneratedArticleMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: ['url2blog', 'articles'] })
    },
  })

  const openGeneratedDeleteModal = (article: Url2BlogSavedArticle) => {
  ...
  const pageContent = articlesQuery.isLoading ? (
    ...
    <main className="u2b-saved-layout">
      <section className="stl-panel">
                      ...
                      <tr key={draft.id}>
                        ...
                        <td>
                          <div className="stl-table-actions">
                            <Link
                              className="stl-link"
                              to={`/url2blog/stage-article?stagedId=${encodeURIComponent(draft.id)}`}
                            >
                              Resume
                            </Link>
                            <button
                      ...
                      <tr key={article.run_id}>
                        ...
                        <td>
                          <div className="stl-table-actions">
                            {localDraft ? (
                              <Link
                                to={`/url2blog/stage-article?stagedId=${encodeURIComponent(localDraft.id)}`}
                                className="stl-link"
                              >
                                Resume
                              </Link>
                            ) : (
                              <span className="u2b-status-note">Synced to Payload</span>
                            )}
    ...
    <div className="stl-page">
      <header className="stl-hero">
        ...
        <div className="stl-hero-actions">
          <Link to="/url2blog" className="stl-btn stl-btn-secondary">Back to Pipeline</Link>
        </div>
      </header>
      {pageContent}
      {generatedDeleteTarget ? (
        <div className="stl-modal-overlay" role="presentation" onClick={closeGeneratedDeleteModal}>
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="url2blog-delete-generated-title"
            onClick={(event) => event.stopPropagation()}
          >
            <h3 id="url2blog-delete-generated-title">Delete Generated Article?</h3>
            <p>
