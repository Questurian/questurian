...
export default function ArticlesPage() {
  ...
  const [generatedDeleteTarget, setGeneratedDeleteTarget] = useState<SavedArticle | null>(null)
  ...
  const articlesQuery = useQuery({
    queryKey: ['youtube2blog', 'articles'],
  ...
  const payloadStatusesQuery = useQuery({
    queryKey: ['youtube2blog', 'payload-statuses', payloadArticleIds.join(','), token || 'no-token'],
  ...
  const deleteGeneratedArticleMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: ['youtube2blog', 'articles'] })
    },
  })

  const openGeneratedDeleteModal = (article: SavedArticle) => {
  ...
  const pageContent = articlesQuery.isLoading ? (
    ...
    <main className="y2b-saved-layout">
      <section className="stl-panel">
                      ...
                      <tr key={draft.id}>
                        ...
                        <td>
                          <div className="stl-table-actions">
                            <Link
                              className="stl-link"
                              to={`/youtube2blog/stage-article?stagedId=${encodeURIComponent(draft.id)}`}
                            >
                              Resume
                            </Link>
                            <button
                      ...
                      <tr key={article.run_id}>
                        ...
                        <td>
                          <div className="stl-table-actions">
                            {localDraft ? (
                              <Link
                                to={`/youtube2blog/stage-article?stagedId=${encodeURIComponent(localDraft.id)}`}
                                className="stl-link"
                              >
                                Resume
                              </Link>
                            ) : (
                              <span className="y2b-status-note">Synced to Payload</span>
                            )}
    ...
    <div className="stl-page">
      <header className="stl-hero">
        ...
        <div className="stl-hero-actions">
          <Link to="/youtube2blog" className="stl-btn stl-btn-secondary">Back to Pipeline</Link>
          <Link to="/youtube2blog/article-types" className="stl-btn stl-btn-secondary">Article Types</Link>
        </div>
      </header>
      {pageContent}
      {generatedDeleteTarget ? (
        <div className="stl-modal-overlay" role="presentation" onClick={closeGeneratedDeleteModal}>
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="youtube2blog-delete-generated-title"
            onClick={(event) => event.stopPropagation()}
          >
            <h3 id="youtube2blog-delete-generated-title">Delete Generated Article?</h3>
            <p>
