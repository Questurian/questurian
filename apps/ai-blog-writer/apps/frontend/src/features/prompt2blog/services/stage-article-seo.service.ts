...
export function buildPrompt2BlogArticleContext(stagedArticle: StagedArticle): string {
...
export function buildPrompt2BlogStructuredDataTemplate(input: {
  stagedArticle: StagedArticle
  locationLabel?: string
}): Record<string, unknown> {
  const { stagedArticle, locationLabel } = input
  const articleBody = buildPrompt2BlogArticleContext(stagedArticle)
...
export function serializePrompt2BlogStructuredDataTemplate(value: Record<string, unknown>): string {
...
export function buildPrompt2BlogSeoAiPrompt(input: {
  location?: string
  target?: SeoAiTarget
}): string {
  ...
  return [
    ...
    '',
    'Article type: prompt2blog standard article',
...
export function validatePrompt2BlogSeoSection(seoSection: SeoSection): string[] {
  const issues: string[] = []
