...
import {
  ...
  normalizeSeoSection
} from './builder/services/seo-section.service'
import {
  DEFAULT_DAY_SHELL_ID,
  BUILT_IN_DAY_SHELLS
} from './builder/constants/day-shells.constants'
import {
  createEmptyDaySlice,
  DEFAULT_LIST_TONE,
...
const DAY_SHELL_IDS = new Set<string>(
  BUILT_IN_DAY_SHELLS.map((shell) => shell.id)
)
const DAYPARTS = new Set<ShellSlotDaypart>([
  'morning',
  'late_morning',
  'lunch',
  'afternoon',
...
const SHELL_COLLECTIONS = new Set([
  'dining',
  'accommodations',
  'attractions',
  'nightlife'
...
function normalizeTravelerProfile(value: unknown): TravelerProfile | undefined {
  ...
  return {
    ...
    budget:
      ...
      TRAVELER_PROFILE_BUDGETS.has(value.budget as TravelerProfileBudget)
    ...
    composedBrief:
      typeof value.composedBrief === 'string' ? value.composedBrief : ''
  }
}
...
function normalizeStoredDraft(
  ...
  const normalizeDayShellSelections = (days: ItineraryDaySlice[]) => {
    const rawSelections = Array.isArray(value.dayShellSelections)
      ? value.dayShellSelections
      : []
    const customShellIds = new Set(
      normalizeCustomDayShells(value.customDayShells).map((shell) => shell.id)
    )
    return days.map((day) => {
      const raw = rawSelections.find(
        (entry) =>
          isRecord(entry) &&
  ...
  const normalizeCustomSlot = (
    ...
    const acceptableCollections = Array.isArray(slotValue.acceptableCollections)
      ? slotValue.acceptableCollections.filter(
          (entry): entry is DayShellSlot['acceptableCollections'][number] =>
            typeof entry === 'string' && SHELL_COLLECTIONS.has(entry)
        )
      : []
    const preferredCollections = Array.isArray(slotValue.preferredCollections)
      ? slotValue.preferredCollections.filter(
          (entry): entry is DayShellSlot['preferredCollections'][number] =>
            typeof entry === 'string' && SHELL_COLLECTIONS.has(entry)
        )
      : acceptableCollections
    if (acceptableCollections.length < 1 || preferredCollections.length < 1)
      return null
    return {
      id:
        typeof slotValue.id === 'string' && slotValue.id.trim()
          ? slotValue.id
          : `custom_slot_${slotIndex}`,
      label:
        typeof slotValue.label === 'string' && slotValue.label.trim()
          ? slotValue.label
  ...
  function normalizeCustomDayShells(shellsValue: unknown): DayShellTemplate[] {
    ...
    return shellsValue
      .map((shellValue, shellIndex): DayShellTemplate | null => {
        ...
        const rawId =
          typeof shellValue.id === 'string' ? shellValue.id.trim() : ''
        const slots = Array.isArray(shellValue.slots)
          ? shellValue.slots
              .map((slotValue, slotIndex) =>
                normalizeCustomSlot(slotValue, slotIndex)
              )
        ...
        return {
          id: rawId,
          name:
            typeof shellValue.name === 'string' && shellValue.name.trim()
              ? shellValue.name.trim()
              : `Custom Shell ${shellIndex + 1}`,
          description:
            typeof shellValue.description === 'string'
              ? shellValue.description.trim()
              : '',
          slots
        }
      })
  ...
  const normalizeStoredKeyLocation = (
    ...
    const relatedItemValue = isRecord(rowValue.relatedItem)
      ? rowValue.relatedItem
      : {}
    const rawRelatedItem = rowValue.relatedItem

    return {
      id:
        typeof rowValue.id === 'string' && rowValue.id.trim()
          ? rowValue.id
      ...
      relatedCollection: isRelatedItemCollection(rowValue.relatedCollection)
        ...
        : isRelatedItemCollection(relatedItemValue.relationTo)
          ...
          : null,
      relatedItem:
        typeof rawRelatedItem === 'number'
          ? rawRelatedItem
          : typeof relatedItemValue.value === 'number'
            ? relatedItemValue.value
            : isRecord(relatedItemValue.value) &&
                typeof relatedItemValue.value.id === 'number'
  ...
  const normalizeStoredStartingPoint = (
    startingPointValue: unknown
    ...
    return {
      label:
        typeof startingPointValue.label === 'string'
          ? startingPointValue.label
          : '',
      latitude:
  ...
  const normalizeStoredItem = (
    itemValue: unknown,
    itemIndex: number
  ): ItineraryItemBlock | null => {
    if (!isRecord(itemValue)) return null

    const itemId =
      typeof itemValue.id === 'string' && itemValue.id.trim()
        ? itemValue.id
    ...
    return {
      ...
      moment: isItineraryMoment(itemValue.moment) ? itemValue.moment : null,
      momentLabel:
        typeof itemValue.momentLabel === 'string' ? itemValue.momentLabel : '',
      tours: Array.isArray(itemValue.tours)
        ? itemValue.tours.filter(
            (entry): entry is number => typeof entry === 'number'
          )
        : [],
      mediaMode:
        itemValue.mediaMode === 'instagram' || itemValue.mediaMode === 'both'
          ? itemValue.mediaMode
          : 'photos',
      selectedPhotos: Array.isArray(itemValue.selectedPhotos)
        ? itemValue.selectedPhotos.filter(
            (entry): entry is number => typeof entry === 'number'
          )
        : [],
      selectedInstagramPost:
        typeof itemValue.selectedInstagramPost === 'number'
          ? itemValue.selectedInstagramPost
          : null,
      title: typeof itemValue.title === 'string' ? itemValue.title : '',
      operator:
        typeof itemValue.operator === 'string' ? itemValue.operator : '',
      ...
      keyLocations: Array.isArray(itemValue.keyLocations)
        ? itemValue.keyLocations
            .map((row, rowIndex) =>
              normalizeStoredKeyLocation(row, itemId, rowIndex)
            )
            .filter((row): row is TourAgencyKeyLocationRow => Boolean(row))
        : [],
      image: typeof itemValue.image === 'number' ? itemValue.image : null,
      instagramPost:
        typeof itemValue.instagramPost === 'number'
          ? itemValue.instagramPost
          : null,
      angle: resolveItineraryAngleForBlockType(blockType, itemValue.angle),
      blurbMarkdown:
        typeof itemValue.blurbMarkdown === 'string'
          ? itemValue.blurbMarkdown
          : '',
      blurbLexical: isRecord(itemValue.blurbLexical)
        ? itemValue.blurbLexical
        : undefined,
      blurbJsonText:
        typeof itemValue.blurbJsonText === 'string'
          ? itemValue.blurbJsonText
          : '',
      selectionReason:
        typeof itemValue.selectionReason === 'string'
          ? itemValue.selectionReason
          : '',
      shellSlotId:
        typeof itemValue.shellSlotId === 'string'
          ? itemValue.shellSlotId
          : undefined,
      shellSlotLabel:
        typeof itemValue.shellSlotLabel === 'string'
          ? itemValue.shellSlotLabel
          : undefined,
      shellSlotDaypart:
  ...
  return {
    ...
    payloadId:
      typeof value.payloadId === 'number' ? value.payloadId : undefined,
    payloadStatus:
      value.payloadStatus === 'published'
        ? 'published'
        : value.payloadStatus === 'draft'
          ? 'draft'
          : undefined,
    payloadSlug:
      typeof value.payloadSlug === 'string' && value.payloadSlug.trim()
        ? value.payloadSlug
        : undefined,
    payloadPublishedAt:
      typeof value.payloadPublishedAt === 'string' &&
      value.payloadPublishedAt.trim()
        ? value.payloadPublishedAt
        : undefined,
    payloadUpdatedAt:
      typeof value.payloadUpdatedAt === 'string' &&
      value.payloadUpdatedAt.trim()
        ? value.payloadUpdatedAt
        : undefined,
    payloadAuthorName:
      typeof value.payloadAuthorName === 'string' &&
      value.payloadAuthorName.trim()
        ? value.payloadAuthorName
        : undefined,
    ...readStoredPayloadSyncFields(value),
    editorModelName: resolveEditorAssistModelName(
      typeof value.editorModelName === 'string'
    ...
    locationRef:
      typeof value.locationRef === 'number' ? value.locationRef : null,
    ...
    header: {
      introMarkdown:
        typeof header.introMarkdown === 'string' ? header.introMarkdown : '',
      introLexical: isRecord(header.introLexical)
        ? header.introLexical
        : undefined,
      introJsonText:
        typeof header.introJsonText === 'string' ? header.introJsonText : '',
      featuredMediaSet:
        typeof header.featuredMediaSet === 'number'
          ? header.featuredMediaSet
          : null,
      featuredImage:
        typeof header.featuredImage === 'number' ? header.featuredImage : null
    },
    ...(() => {
      if (Array.isArray(value.days) && value.days.length > 0) {
        const days: ItineraryDaySlice[] = value.days.map(
          (dayVal: unknown, dayIndex: number) => {
            if (!isRecord(dayVal)) {
              return {
                ...createEmptyDaySlice(),
            ...
            const rawWhere = Array.isArray(dayVal.whereStaying)
              ? dayVal.whereStaying
              : []
            ...
            const mappedItems = rawItems
              .map((item, itemIndex) => normalizeStoredItem(item, itemIndex))
              .filter((item): item is ItineraryItemBlock => Boolean(item))
            const lodgingFromItems = mappedItems.filter(
              (item) => item.blockType === 'itinerary-where-staying'
            )
            const stops = mappedItems.filter(
              (item) => item.blockType !== 'itinerary-where-staying'
            )
            const dayId =
              typeof dayVal.id === 'string' && dayVal.id.trim()
                ? dayVal.id
      ...
      const rawWhere = Array.isArray(value.whereStaying)
        ? value.whereStaying
        : []
      ...
      return {
        ...
        dayShellSelections: [
          { dayId: `day_${normalizedDraftId}_0`, shellId: DEFAULT_DAY_SHELL_ID }
        ]
      }
    })(),
    seoSection: normalizeSeoSection(
      value.seoSection ?? createEmptySeoSection()
    ),
