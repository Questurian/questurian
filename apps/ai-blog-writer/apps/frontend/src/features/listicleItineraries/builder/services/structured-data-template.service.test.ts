...
function buildDraft(): ListicleItineraryDraft {
  return {
    ...
    header: {
      introMarkdown:
        'Discover an unforgettable day in Lima with this long intro that should still be normalized for structured data.',
      introJsonText: '',
      featuredImage: null
    },
    dayCount: 1,
...
function buildRelatedByType(): Record<
...
describe('listicleItineraries structured data template', () => {
  it('builds valid published structured data with trip metadata', () => {
    ...
    const graph = Array.isArray(structuredData['@graph'])
      ...
      : []
    const blogPostingNode = graph.find(
      (node) =>
        node &&
        typeof node === 'object' &&
        (node as Record<string, unknown>)['@type'] === 'BlogPosting'
    ) as Record<string, unknown> | undefined
    const tripNode = graph.find(
      (node) =>
        node &&
        typeof node === 'object' &&
        (node as Record<string, unknown>)['@type'] === 'TouristTrip'
    ) as Record<string, unknown> | undefined
    const itemListNode = graph.find(
      (node) =>
        node &&
        typeof node === 'object' &&
    ...
    const author =
      blogPostingNode?.author && typeof blogPostingNode.author === 'object'
        ? (blogPostingNode.author as Record<string, unknown>)
        : undefined
    expect(author?.name).toBe('Alan Malpartida')

    const publisher =
      blogPostingNode?.publisher &&
      typeof blogPostingNode.publisher === 'object'
        ? (blogPostingNode.publisher as Record<string, unknown>)
        : undefined
    expect(publisher?.name).toBe('Questurian')

    const contentLocation =
      blogPostingNode?.contentLocation &&
      typeof blogPostingNode.contentLocation === 'object'
    ...
    const mainEntityOfPage =
      blogPostingNode?.mainEntityOfPage &&
      typeof blogPostingNode.mainEntityOfPage === 'object'
        ? (blogPostingNode.mainEntityOfPage as Record<string, unknown>)
        : undefined
    expect(mainEntityOfPage?.['@id']).toBe(
      'https://example.com/one-day-lima-itinerary'
    )

    const itemListElement = Array.isArray(itemListNode?.itemListElement)
      ? itemListNode.itemListElement
      : []
    const firstListItem = itemListElement[0] as
      | Record<string, unknown>
      | undefined
    const secondListItem = itemListElement[1] as
      | Record<string, unknown>
      | undefined
    const firstEntity =
      firstListItem?.item && typeof firstListItem.item === 'object'
        ? (firstListItem.item as Record<string, unknown>)
        : undefined
    const secondEntity =
      secondListItem?.item && typeof secondListItem.item === 'object'
    ...
    expect(
      typeof firstEntity?.keywords === 'string' ? firstEntity.keywords : ''
    ).toContain('Dining')
  ...
  it('serializes manual tour-agency key locations and instagram permalink', () => {
        ...
        items: [
          {
            ...
            instagramPost: 42,
            blurbMarkdown:
              'A scenic full-day circuit through the Sacred Valley.',
            blurbJsonText: ''
          }
        ]
      }
    ...
    const manualStopEntity = manualStop?.item as
      | Record<string, unknown>
      | undefined
    const itinerary = manualStopEntity?.itinerary as
      | Record<string, unknown>
      | undefined
    const itineraryStops = Array.isArray(itinerary?.itemListElement)
      ? itinerary.itemListElement
      : []
    const secondRoutePoint = itineraryStops[1] as
      | Record<string, unknown>
      | undefined
    const secondRoutePlace = secondRoutePoint?.item as
      | Record<string, unknown>
      | undefined
    const secondRouteGeo = secondRoutePlace?.geo as
      | Record<string, unknown>
      | undefined
    const departureLocation = manualStopEntity?.departureLocation as
      | Record<string, unknown>
      | undefined
    const departureGeo = departureLocation?.geo as
      | Record<string, unknown>
      | undefined

    expect(manualStopEntity?.sameAs).toEqual([
  ...
  it('reports broken graph references and invalid stop entities', () => {
    const draft = buildDraft()
    const structuredData = buildListicleItineraryStructuredDataTemplate({
      draft,
      relatedByBlockType: buildRelatedByType(),
    ...
    const itemListNode = graph.find((node) => node['@type'] === 'ItemList')
    const itemListElements = itemListNode?.itemListElement as Array<
      Record<string, unknown>
    >
    const firstStop = itemListElements[0].item as Record<string, unknown>

    tripNode!.itinerary = { '@id': '#wrong-item-list' }
    firstStop['@type'] = 'Event'
    firstStop.name = ' '
    ...
    expect(
      ...
      expect.arrayContaining([
        ...
        'Structured Data itemListElement[0].item must include an allowed "@type" for this stop. Prefer "Restaurant".',
        'Structured Data itemListElement[0].item must include a non-empty name.'
      ])
    )
  })
})
