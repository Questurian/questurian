...
describe('listicleItineraries storage', () => {
  ...
  it('migrates legacy Payload sync field names on read', () => {
    const legacyDraft = createEmptyDraft() as ReturnType<
      typeof createEmptyDraft
    > & {
  ...
  it('round-trips manual tour-agency instagram and key-location rows', () => {
        ...
        items: [
          {
            ...
            item: null,
            moment: 'culture',
            momentLabel: 'Sacred Valley culture',
            tours: [],
            mediaMode: 'photos',
            selectedPhotos: [],
            ...
            instagramPost: 42,
            blurbMarkdown:
              'A curated half-day route through the Sacred Valley.',
            blurbJsonText: ''
          }
        ]
      }
    ...
    expect(restored?.days[0]?.items[0]?.moment).toBe('culture')
    expect(restored?.days[0]?.items[0]?.momentLabel).toBe(
      'Sacred Valley culture'
    )
  ...
  it('drops legacy schedule fields while preserving current-key drafts', () => {
    ...
    expect(restored?.days[0]?.items[0]?.title).toBe(
      'Legacy Sacred Valley Circuit'
    )
    expect('itineraryStartHour' in (restored as Record<string, unknown>)).toBe(
      false
    )
    expect(
      'timeHour' in
        ((restored?.days[0]?.items[0] ?? {}) as Record<string, unknown>)
    ).toBe(false)
  })
