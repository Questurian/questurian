"""Report one external call to the dashboard's API-usage collector.

Every pipeline in this backend spends money and time in other people's
services -- Vertex, the Anthropic API, the Claude CLI, Places, image
providers -- and until now the only accounting was Prompt2Blog's per-run token
ledger. That ledger answers "what did this run cost". It cannot answer "how
often does Vertex fail", "which stage is slow" or "what did we spend last
week", because it is scoped to one run, records nothing about duration or
failures, and never sees a non-LLM call.

This module reports each call to the collector in `apps/dashboard`, which owns
the storage and the UI. It is deliberately a *report*, not a dependency:
...
The token ledger stays exactly where it is. This is a second reader of the same
facts, not a replacement: the ledger's per-run receipt and the collector's
cross-app history answer different questions, and merging them would make
neither trustworthy.

Why ``app.shared`` and not ``packages/utils``
---------------------------------------------
Several tests replace ``utils`` in ``sys.modules`` with a stub, and that stub is
process-global. A new name in ``utils`` imported at module scope by a pipeline
breaks every test that imports after one of those stubs is installed. See
``app/shared/provider_faults.py``, which lives here for the same reason.

Configuration (all optional -- with no URL set, this module does nothing):

``USAGE_MONITOR_URL``      the collector's ingest endpoint,
                           e.g. ``http://localhost:4500/api/usage/v1/events``
``USAGE_MONITOR_KEY``      sent as ``x-usage-key`` when the collector requires one
``USAGE_MONITOR_SERVICE``  the name this app reports itself as (default ``abw-backend``)
...
from app.shared.token_usage import (
    VERTEX_TOKEN_RATES,
...
_PROVIDER_BY_LLM_CLASS = {
    ...
    "VertexTextLLM": PROVIDER_GOOGLE_VERTEX,
...
UNPRICEABLE_PROVIDERS = frozenset({PROVIDER_CLAUDE_CLI})
