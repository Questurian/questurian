...
**Note (2026-07-08):** two convergences between this ADR and the implementation.

1. **`idealFor` provenance is per-field, not per-tag.** §2's `provenance.idealFor[<tag>]` shape was never implemented; the live sidecar is a flat `field → FieldProvenance` map (`LocationProvenance` in `lm-shared/types/location-provenance.ts`) and stays that way. An operator edit to any part of the tag set demotes the whole `idealFor` field to operator. The mixed-provenance badge rendering §2 describes never shipped either — the detail page renders one badge for the whole row — so per-tag audit granularity buys machinery without a consumer. If per-tag audit becomes a real need, reopen with a shape migration.
2. **Flip-to-operator is enforced server-side.** §2's invariant ("operator edits flip the corresponding entry to `operator`") was previously enforced only in the Add-flow client state; edits to an existing Location via PATCH left stale `google`/`ai`/`scraper` tags on hand-edited values. The update path (`update-maps-location.operation.ts` → `utils/location-utils/provenance.ts`) now demotes any provenance-tracked field whose persisted value changes. Demotion removes the entry: `operator` is the implicit default for absent entries. Sidecar parsing/serialization is owned by `lm-shared` (`parseLocationProvenanceJson` / `serializeLocationProvenanceJson` / `demoteToOperator` / `sanitizeLocationProvenance`); callers must not hand-roll the JSON.
