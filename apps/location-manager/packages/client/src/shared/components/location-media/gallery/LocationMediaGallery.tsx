...
export function LocationMediaGallery({ locationDetail }: LocationMediaGalleryProps) {
  ...
  const instagramStagingActive = (locationDetail.instagram_embeds || []).some((embed) =>
    embed.media_staging_status === "pending" || embed.media_staging_status === "processing"
  );
  return (
    <>
      {(gallery.uploadsWithPreview.length > 0 || stagedSourceCount > 0 || instagramStagingActive || !!locationDetail.placeId || gallery.isAttraction) && (
        <div className="space-y-2">
          ...
          <PhotoImportPanel locationId={locationDetail.id} category={locationDetail.category} placeId={locationDetail.placeId ?? null} instagramStagingActive={instagramStagingActive} />
