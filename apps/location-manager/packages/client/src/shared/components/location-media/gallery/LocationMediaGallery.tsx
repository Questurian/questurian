...
export function LocationMediaGallery({ locationDetail }: LocationMediaGalleryProps) {
  ...
  return (
    <>
      {(gallery.uploadsWithPreview.length > 0 || gallery.isAttraction) && (
      ...
      <InstagramEmbedsGallery embeds={gallery.instagramEmbedsWithPreview} onOpen={gallery.openInstagram} onDelete={(id) => gallery.setDeleteConfirm({ type: "instagram", id })} />
      <PhotoImportPanel locationId={locationDetail.id} category={locationDetail.category} placeId={locationDetail.placeId ?? null} />
