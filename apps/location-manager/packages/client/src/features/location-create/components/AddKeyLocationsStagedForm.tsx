...
import { CheckCircle2, ChevronLeft, Clock, Route } from "lucide-react";
...
type KeyLocationsFormSection = "step1" | "entities" | "profile" | "operations";

const KEY_LOCATIONS_SECTION_ORDER: KeyLocationsFormSection[] = [
  "step1",
  "entities",
  "profile",
  "operations",
...
const STATUS_OPTIONS: Array<{ value: AddKeyLocationsFormData["status"]; label: string }> = [
...
interface AddKeyLocationsStagedFormProps {
  form: UseFormReturn<AddKeyLocationsFormData>;
  onSubmit: (data: AddKeyLocationsFormData) => void;
...
export function AddKeyLocationsStagedForm({
  form,
  onSubmit,
  onRunGooglePrefill,
  isPrefillingGoogle,
  ...
  locationTypes,
  isLoadingTypes,
}: AddKeyLocationsStagedFormProps) {
  const [activeSection, setActiveSection] = useState<KeyLocationsFormSection>("step1");
  ...
  const flowSections: Array<{
    ...
    { key: "operations", label: "Operations", complete: operationsComplete },
  ];

  const canOpenSection = (section: KeyLocationsFormSection) => {
    if (section === "step1") return true;
    return isPrefillReady;
  };

  const goToSection = (section: KeyLocationsFormSection) => {
    if (!canOpenSection(section)) return;
    setActiveSection(section);
  };

  const goToPreviousSection = () => {
    const currentIndex = KEY_LOCATIONS_SECTION_ORDER.indexOf(activeSection);
    const previousSection = KEY_LOCATIONS_SECTION_ORDER[currentIndex - 1];
    if (previousSection) {
  ...
  const goToNextSection = async () => {
    const currentIndex = KEY_LOCATIONS_SECTION_ORDER.indexOf(activeSection);
    const nextSection = KEY_LOCATIONS_SECTION_ORDER[currentIndex + 1];
    ...
    if (activeSection === "profile") {
      const isValid = await form.trigger(["type", "status"]);
      if (!isValid) return;
    }
      ...
      <div className="w-full max-w-6xl rounded-2xl border border-border/80 bg-card p-5 shadow-sm sm:p-6">
        <div className="mx-auto w-full max-w-5xl">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        form.setValue("status", event.target.value as AddKeyLocationsFormData["status"], {
                          shouldDirty: true,
                          shouldValidate: true,
                          shouldTouch: true,
                        })
            ...
            {isPrefillReady && activeSection === "operations" && (
              <section className="space-y-4 rounded-xl border border-border/70 bg-background/20 p-4 sm:p-5">
                <h2 className="text-lg font-semibold tracking-tight text-foreground">Operations & Contact</h2>
                ...
                <div className="flex justify-between border-t border-border/70 pt-4">
                  ...
                  <Button type="submit" disabled={!isPrefillReady || !form.formState.isValid || isCreating}>
                    {isCreating ? "Creating..." : "Create Key Location Document"}
                  </Button>
                </div>
              </section>
            )}
