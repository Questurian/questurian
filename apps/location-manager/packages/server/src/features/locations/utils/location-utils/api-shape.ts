...
import { assertCategory, stripNightlifeSpendLevel } from "./category";
import { formatLocationForDisplay } from "./parse";
...
export function transformLocationToBasicResponse(
  location: Location & { uploadsCount: number; instagramEmbedsCount: number }
): LocationBasic {
  const category = assertCategory(location.category);
