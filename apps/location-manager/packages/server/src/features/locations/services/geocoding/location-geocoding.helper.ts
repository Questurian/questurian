...
interface NormalizedOperationHours {
...
function normalizeGoogleWeekdayTextToOperationHours(
  ...
  for (const rawLine of weekdayText) {
    ...
    const [rawDay, rawHours] = normalizedLine.split(/:\s*/, 2);
    if (!rawDay || rawHours === undefined) continue;
    ...
    for (const segment of segments) {
      ...
      hoursByDay.get(dayIndex)!.push(`${open} - ${close}`);
    }
  }
  ...
  return {
    hours: WEEKDAY_NAMES.map((dayName, index) => {
      ...
      return {
        day: dayName,
        hours: slots.length > 0 ? slots.join(", ") : "Closed",
      };
    }),
  };
}
...
function normalizeGoogleOpeningHours(
  ...
  for (const period of periods) {
    ...
    if (typeof openDay !== "number" || openDay < 0 || openDay > 6 || !openTime) {
      continue;
    }
  ...
  const hasPeriodSlots = daySlots.some((slots) => slots.length > 0);
  if (!hasPeriodSlots) {
  ...
  try {
    ...
    if (searchData.status === "OK" && searchData.results && searchData.results.length > 0) {
      ...
      if (detailsData.status === "OK" && detailsData.result) {
        // Include the place_id in the result
        return {
          ...detailsData.result,
          place_id: placeId,
        };
    ...
    try {
      ...
      if (placeDetails) {
        ...
        if (options?.includeOperationHours && placeDetails.opening_hours) {
          ...
          if (operationHours) {
            entry.hoursJson = JSON.stringify(operationHours);
          }
        }
