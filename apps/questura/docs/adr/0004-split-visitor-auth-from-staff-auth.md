...
PostgreSQL owns the atomic invariant. `identity_email_owners` is an auth-infrastructure registry keyed by normalized email; triggers on Payload `users` and BetterAuth `visitor_auth_users` register inserts and email changes in the same transaction, and release ownership on deletion. Its single primary key closes the simultaneous cross-surface creation race that independent application checks cannot close. Visitor registry rows also reject the reserved Staff domain. Application guards remain because they provide earlier, domain-specific errors; the registry is the final race-safe boundary.
