 ...
 *
 * `launch:verify --local` installs this before any request. `net.connect`
 * reads `dns.lookup` at call time, so fetch picks it up; `dns.promises.lookup`
 * is covered too. Every other name still goes to the OS resolver untouched.
 */
