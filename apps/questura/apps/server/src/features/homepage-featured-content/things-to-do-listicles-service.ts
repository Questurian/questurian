...
const ATTRACTION_LISTICLE_TYPE = 'attractions'
...
export function normalizeThingsToDoListiclesInput(rawItems: unknown): HomepageFeaturedItemRef[] {
...
export function buildThingsToDoListiclesGlobalData(items: HomepageFeaturedItemRef[]) {
...
export async function validateThingsToDoListiclesItems(
  ...
  await Promise.all(
    validated.map(async (ref) => {
      ...
      if (doc.listicleType !== ATTRACTION_LISTICLE_TYPE) {
        const title = typeof doc.title === 'string' && doc.title.trim() ? doc.title : `#${ref.id}`
        throw new Error(`"${title}" is not an attractions listicle and cannot be used in this block.`)
      }
    }),
  )
  ...
  const docs = allowedIds.size > 0
    ? await payload.find({
        ...
        where: {
          and: [
            { id: { in: [...allowedIds] } },
            { listicleType: { equals: ATTRACTION_LISTICLE_TYPE } },
          ],
        },
        overrideAccess: true,
      })
