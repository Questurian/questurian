...
type ListicleBlockSlug =
  | 'data-dining'
  | 'data-accommodations'
  | 'data-attractions'
  | 'data-nightlife'

export type ListicleSourceCollection =
  | 'dining'
  | 'accommodations'
  | 'attractions'
  | 'nightlife'
...
export const blockSlugToSourceCollection: Record<ListicleBlockSlug, ListicleSourceCollection> = {
  'data-dining': 'dining',
...
export const getSourceCollectionForBlockType = (
  blockType: unknown,
): ListicleSourceCollection | null => {
  if (typeof blockType !== 'string') {
    return null
  }

  return blockSlugToSourceCollection[blockType as ListicleBlockSlug] ?? null
}
...
export const fetchListicleSourceItem = async (
  req: PayloadRequest,
  sourceCollection: ListicleSourceCollection,
...
export const createSelectedPhotosFilterOptions = (sourceCollection: ListicleSourceCollection) =>
...
export const createItemMediaFields = (sourceCollection: ListicleSourceCollection): Field[] => [
  {
    name: 'mediaMode',
    type: 'select',
    dbName: 'mm',
    enumName: 'stl_media_mode',
    required: true,
    options: mediaModeOptions,
    admin: {
      description: 'Select whether this list item uses photos, Instagram, or both.',
    },
  },
  {
    name: 'selectedPhotos',
    ...
    admin: {
      description: 'Select 1 to 6 photos from the selected source item gallery.',
  ...
  },
  {
    ...
    admin: {
      allowCreate: true,
      description: 'Select one Instagram embed from the selected source item.',
