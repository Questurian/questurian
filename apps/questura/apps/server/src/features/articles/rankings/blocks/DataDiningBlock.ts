/**
 * Data Dining Block
 * Block type for selecting dining items from the data collection
 * References Dining collection with single image selection
 */
...
export const DataDiningBlock: Block = {
  ...
  label: ({ data }) => {
    if (data?.item && typeof data.item === 'object' && 'title' in data.item) {
      return (data.item as { title: string }).title || 'Dining Item'
    }
    return 'Dining Item'
  },
  fields: [
    {
      ...
      admin: {
        description: 'Select a dining establishment from the data collection',
        components: {
          Field: 'src/features/articles/rankings/components/collection-wrappers/item-fields/DiningItemField.tsx',
        },
      },
    },
    {
      ...
      admin: {
        ...
        components: {
          Field: 'src/features/articles/rankings/components/collection-wrappers/gallery-pickers/DiningGalleryImagePicker.tsx',
        },
      },
      validate: (value) => {
    ...
    },
    {
      ...
      required: true,
      admin: {
        description: 'Editorial content or commentary about this dining venue',
      },
    },
  ],
}
