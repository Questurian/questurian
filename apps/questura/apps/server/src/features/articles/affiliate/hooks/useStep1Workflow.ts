 ...
 *
 * Encapsulates the state machine logic for the multi-step affiliate article form workflow.
 *
 * Responsibilities:
 * - Manage Step 1 field validation (title only)
...
export const useStep1Workflow = (): Step1WorkflowState => {
  ...
  // 1. READ FORM STATE
  // ===========================
  const { title, step1_complete, in_update_mode } = useFormFields(
    ([fields]) => ({
