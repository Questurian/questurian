...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const response = await searchWhereToEatDrinkCandidates(payload, {
    ...
    return NextResponse.json({ message: getErrorMessage(error, 'Failed to load Where to Eat & Drink candidates.') }, { status: 500, headers })
  }
}
