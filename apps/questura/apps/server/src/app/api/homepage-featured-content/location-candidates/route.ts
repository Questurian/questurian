...
import { MAIN_HOMEPAGE_LOCATION_GRID_SCOPE, searchLocationGridCandidates } from '@/features/homepage-featured-content'
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const response = await searchLocationGridCandidates(payload, {
      ...
      scope: MAIN_HOMEPAGE_LOCATION_GRID_SCOPE,
    })
    return NextResponse.json(response, { headers })
  } catch (error) {
    return NextResponse.json({ message: getErrorMessage(error, 'Failed to load location grid candidates.') }, { status: 500, headers })
  }
}
