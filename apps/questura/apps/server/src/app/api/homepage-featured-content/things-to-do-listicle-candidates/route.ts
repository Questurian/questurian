...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const response = await searchThingsToDoListicleCandidates(payload, {
    ...
    return NextResponse.json({ message: getErrorMessage(error, 'Failed to load Things to Do listicle candidates.') }, { status: 500, headers })
  }
}
