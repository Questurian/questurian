...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const response = await searchThingsToDoAttractionCandidates(payload, {
    ...
    return NextResponse.json({ message: getErrorMessage(error, 'Failed to load Things to Do attraction candidates.') }, { status: 500, headers })
  }
}
