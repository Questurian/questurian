...
import { identityStore, PRIMED_IDENTITY_MAX_AGE_MS } from '@/lib/user/currentIdentity';
 ...
 * joined, not repeated. `maxAgeMs` is small — long enough to reuse the primed
 * answer on a slow hydration, short next to React Query's own `staleTime`,
 * which decides when the navbar re-asks.
 */
async function getCurrentUser(options: { maxAgeMs?: number } = {}): Promise<User | null> {
  const response = await identityStore.read({ maxAgeMs: options.maxAgeMs ?? PRIMED_IDENTITY_MAX_AGE_MS });
