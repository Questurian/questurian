...
import { AuthorFeaturePreview } from "../components/blocks/author-feature/AuthorFeaturePreview";

function homepageBlockLayoutKey(
  blockType: string,
  totalSlots: number,
): HomepageBlockLayoutKey {
...
export function defineHomepageBlockLayoutAnySlots<
  TBlock extends CityHomepageBlock,
>(
...
export function getHomepageBlockTotalSlots(
  block: CityHomepageBlock,
): number | null {
  const totalSlots =
    "totalSlots" in block ? block.totalSlots : block.selection?.totalSlots;
...
const homepageBlockLayoutMap = new Map<
  HomepageBlockLayoutKey,
  HomepageBlockLayoutDefinition
>(
...
const homepageBlockFallbackLayouts: HomepageBlockLayoutFallbackDefinition[] = [
  ...
  defineHomepageBlockLayoutAnySlots<AuthorFeatureBlock>({
    blockType: "author-feature",
    Component: AuthorFeaturePreview,
  }),
...
const homepageBlockFallbackMap = new Map<
  string,
  HomepageBlockLayoutFallbackDefinition
...
export function getHomepageBlockLayout(
  ...
  if (totalSlots !== null) {
    const exact = homepageBlockLayoutMap.get(
      homepageBlockLayoutKey(block.blockType, totalSlots),
    );
