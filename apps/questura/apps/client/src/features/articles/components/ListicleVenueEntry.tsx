...
export function ListicleVenueEntry({
  ...
  const addressRaw =
    typeof row.item.address === "string" ? row.item.address.trim() : "";
  ...
  const itineraryBlurbHtml =
    isItinerary && (blurb || idealForProse)
      ? `${blurb ?? ""}${idealForProse}`
      : null;
            ...
            <p className="maps-listicle-meta-row">
              {metaParts.map((part, i) => (
                <span key={`${part}-${i}`} className="maps-listicle-meta-item">
                  {i > 0 ? (
                    <span
                      className="maps-listicle-meta-separator"
                      aria-hidden
                    />
                  ) : null}
                  <span className="maps-listicle-meta-value">{part}</span>
                </span>
              ))}
            </p>
          ) : null}
