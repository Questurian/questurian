...
- `PAYLOAD_COOKIE_DOMAIN` is required to boot in production. **Operator tools** run on sibling subdomains of the registrable domain, and a host-only `payload-token` is never sent to them, so **Staff auth** there fails as an unexplained 401. A genuinely same-origin deployment sets `host-only` to say so; an unset variable is an oversight, not a default. `PAYLOAD_COOKIE_REQUIRED_HOSTS` names the hosts that must receive the session, and boot refuses a domain that does not reach all of them plus Payload's own host — scoping it to Payload's host alone is self-consistent and still produces the silent 401. That list is deliberately not `CORS_ORIGINS`: an allowed browser origin is not necessarily a host that should hold a **Staff session**, and `abw-api` holds one without ever being a browser origin.
